...
const NexicapsAI = {
    ...
    async generateExam(input) {
        try {
            const response = await fetch('http://localhost:3000/api/generate/exam', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(input)
            });
    ...
    async generateWorksheet(input) {
        try {
            const response = await fetch('http://localhost:3000/api/generate/worksheet', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(input)
            });
    ...
    async generateImage(prompt) {
        try {
            const response = await fetch(`http://localhost:3000/api/generate/image?prompt=${encodeURIComponent(prompt)}`);
            if (!response.ok) throw new Error('Network response was not ok');
            const data = await response.json();
            return data.url;
        } catch (error) {
    ...
    async generateChat(messages) {
        try {
            const response = await fetch('http://localhost:3000/api/generate/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
